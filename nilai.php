...
$soal = mysqli_query($db,"SELECT * FROM quiz_pilganda where id_tq='$_POST[id_topik]'");
$pilganda = mysqli_num_rows($soal);
$soal_esay = mysqli_query($db,"SELECT * FROM quiz_esay WHERE id_tq='$_POST[id_topik]'");
...
if(!empty($_POST['soal_pilganda'])){
    ...
    $cek = mysqli_query($db,"SELECT * FROM quiz_pilganda WHERE id_quiz=$key");
...
mysqli_query($db,"INSERT INTO nilai (id_tq, id_siswa, benar, salah, tidak_dikerjakan,persentase)
...
if(!empty($_POST['soal_esay'])){
    ...
    $jawaban = $value;
    $cek = mysqli_query($db,"SELECT * FROM quiz_esay WHERE id_quiz=$key2");
    while($data = mysqli_fetch_array($cek)){
        mysqli_query($db,"INSERT INTO jawaban(id_tq,id_quiz,id_siswa,jawaban)
