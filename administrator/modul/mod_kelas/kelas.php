...
switch($_GET[act]){
  ...
  default:
    ...
    elseif ($_SESSION[leveluser]=='siswa'){
        ...
        $kelas = mysqli_query($db,"SELECT * FROM kelas WHERE id_kelas = '$data_siswa[id_kelas]'");
    ...
    echo "<form method=POST action='$aksi?module=kelas&act=input_kelas'>
          ...
          <dt><label>Wali Kelas</label></dt>      <dd> : <select name='id_pengajar'>
                                      ...
                                      $tampil=mysqli_query($db,"SELECT * FROM pengajar ORDER BY nama_lengkap");
                                      while($r=mysqli_fetch_array($tampil)){
          ...
          <dt><label>Ketua Kelas</label></dt>     <dd> : <select name='id_siswa'>
                                      ...
                                      $tampil_siswa=mysqli_query($db,"SELECT * FROM siswa ORDER BY nama_lengkap");
    ...
    elseif ($_SESSION[leveluser]=='pengajar'){
        echo "<form name='form_kelas' method=POST action='$aksi?module=kelas&act=input_walikelas'>
          ...
          <dt><label>Kelas </label></dt>    <dd> <select name='kelas' onChange='showsiswa()'>
                                      ...
                                      $tampilk = mysqli_query($db,"SELECT * FROM kelas WHERE id_pengajar ='0' ORDER BY id_kelas");
    ...
    case "editkelas":
    if ($_SESSION[leveluser]=='admin'){
    $tampil = mysqli_query($db,"SELECT * FROM kelas WHERE id = '$_GET[id]'");
    $r = mysqli_fetch_array($tampil);
    $getnip = mysqli_query($db,"SELECT * FROM pengajar WHERE id_pengajar = '$r[id_pengajar]'");
    $nipp = mysqli_fetch_array($getnip);
    $getnis = mysqli_query($db,"SELECT * FROM siswa WHERE id_siswa = '$r[id_siswa]'");
    ...
    <input type=hidden name=id value='$r[id]'>
          ...
          <dt><label>Ketua Kelas</label></dt> <dd>:  <select name='ketua'>";
                                      ...
                                      $tampil_siswa = mysqli_query($db,"SELECT * FROM siswa WHERE id_kelas = '$r[id_kelas]'");
                                      while($s=mysqli_fetch_array($tampil_siswa)){
                                          echo "<option value=$s[id_siswa]>$s[nama_lengkap]</option>";
                                      }echo"</select></dd>
         <p align=center><input type=submit class='button blue' value=Simpan>
    ...
    elseif ($_SESSION[leveluser]=='siswa'){
         ...
         echo "<table>
          <tr><td>Kelas </td>   <td>: <select name='id_kelas'>
                                      <option value='$r[id_kelas]' selected>$r[nama]</option>";
                                      $tampilk = mysqli_query($db,"SELECT * FROM kelas ORDER BY id_kelas");
                                      while($t=mysqli_fetch_array($tampilk)){
                                            echo "<option value=$t[id_kelas]>$t[nama]</option>";
                                      }echo"</select></td></tr>
...
case "detailkelas":
    $detail=mysqli_query($db,"SELECT * FROM kelas WHERE id_kelas='$_GET[id]'");
    ...
    while ($r=mysqli_fetch_array($detail)){
       echo "<tr>
             <td>$r[id_kelas]</td>
             <td>$r[nama]</td>";
             $getpengajar = mysqli_query($db,"SELECT * FROM pengajar WHERE id_pengajar = '$r[id_pengajar]'");
             ...
             $getsiswa = mysqli_query($db,"SELECT * FROM siswa WHERE id_siswa = '$r[id_siswa]'");
